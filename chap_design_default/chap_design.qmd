...
:-------------------------:|:------------------------------------------:
![](fig/RAFisherSmoking.png){width="50%"}|  *To consult the statistician after an experiment is finished is often merely to ask him to conduct a post mortem examination. He can perhaps say what the experiment died of.*
...
library("hexbin")
data("airway")
aw = DESeqDataSet(airway, design = ~ cell + dex) %>%
  estimateSizeFactors
sizeFactors(aw)
...
::: {.column width="20%"}
![](fig/balancechem.png){fig-align="right" width="50%"} 
:::

::: {.column width="80%"}

A pharmacist's balance weighing analogy (@Hotelling1944 and @Mood1946). 
:::
:::
...
```{r bootstrapHotelling, fig.height=7, out.width = "100%"}
...
library("ggplot2")
library("tibble")
ggplot(tibble(ratio = errs[1,] / errs[2,]), 
       aes(x = log2(ratio))) +
...
```{r block, fig.width = 4, fig.height = 2, out.width="100%"} 
#| echo: false
