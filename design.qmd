...
# Data transformations ($\log(x)$ et al.)

Why?

\begin{equation}
Y_{\text{obs}} = f(\theta) + \varepsilon
\end{equation}
...
- Fluorescence based assays: often additive + multiplicative [@RockeDurbin:2001] 
- RNA-Seq: Poisson + multiplicative.


![](fig/delta_method_modified.png){fig-align="center" width="100%"}
...
A single cell expression data matrix (Hiiragi2013 package) data (only tfirst 5 rows and columns):
